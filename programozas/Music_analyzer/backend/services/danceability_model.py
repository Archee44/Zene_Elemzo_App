...
DANCE_DIR = os.path.join(MODEL_DIR, 'danceability')
_EFFNET_PRED = None
_HEAD_CACHE = {}
...
def _find_head_files() -> Optional[Dict[str, str]]:
    ...
    for name in os.listdir(DANCE_DIR):
        if name.endswith('.pb') and 'danceability' in name:
            lower = name.lower()
            kind = 'regression' if 'regression' in lower else ('3c' if '3c' in lower else ('2c' if '2c' in lower else 'cls'))
            pb = os.path.join(DANCE_DIR, name)
            base = name.rsplit('.', 1)[0]
            json_match = os.path.join(DANCE_DIR, base + '.json')
            json_any = None
            for j in os.listdir(DANCE_DIR):
                if j.endswith('.json') and 'danceability' in j:
                    json_any = os.path.join(DANCE_DIR, j)
                    if j == base + '.json':
                        json_match = json_any
                        break
...
def predict_danceability(filename: str) -> Optional[Dict]:
    if es is None:
        return None
    ...
    try:
        ...
        if clf is None:
            for inp_name, out_name in io_candidates:
                try:
                    ...
                    continue
        if out is None:
            raise last_err or RuntimeError('No valid IO nodes for danceability head')
        ...
        kind = head['kind']
        model_name = f"danceability-{kind}"
        ...
        print('[danceability_model] inference error:', str(e))
        return None
